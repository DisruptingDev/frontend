...
export default function DataTable() {
  ...
  const handleDownload = async (id) => {
    let xmlContent, htmlContent;
  
    // Descargar el XML
    try {
      const token = localStorage.getItem('authToken'); // Asumiendo que necesitas un token
      const response = await fetch(`http://31.220.31.152:8090/DescargaXML/${id}`, {
        headers: {
      ...
      if (response.ok) {
        console.log('Data received from API (xml):', response);
        xmlContent = await response.text(); // Aquí se asigna correctamente a xmlContent
      } else {
        console.log('Error al descargar el XML:', response);
        console.error('Error al descargar el XML:', response.statusText);
      }
    } catch (error) {
